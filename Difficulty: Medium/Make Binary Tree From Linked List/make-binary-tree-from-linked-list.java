...
class Solution {
    ...
    public static Tree constructTree(List<Integer> list, int index, Tree node) {
        if (index >= list.size()) return null;
         node = new Tree(list.get(index));
        node.left = constructTree(list, 2 * index + 1, node.left);
        node.right = constructTree(list, 2 * index + 2, node.right);
        return node;
    }
}
